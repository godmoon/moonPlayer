...
export async function filesRoutes(app) {
    ...
    app.get('/api/browse', async (req, reply) => {
        ...
        if (!dir) {
            ...
            return {
                ...
                rootPath: '',
                parentPath: null,
                directories: roots.filter((r) => r.valid),
                files: [],
                isRootsView: true
            };
        }
        ...
        const isInAllowedPath = rootPaths.some((rp) => {
        ...
        // 找到当前路径所属的根目录
        const currentRoot = rootPaths.find((rp) => {
            const resolvedRoot = path.resolve(rp);
    ...
    app.get('/api/scan', async (req, reply) => {
        const { dir, recursive = 'true' } = req.query;
        if (!dir) {
