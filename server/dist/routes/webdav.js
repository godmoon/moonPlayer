...
export async function webdavRoutes(app) {
    const db = getDatabase();
    // 获取所有 WebDAV 配置
    app.get('/api/webdav', async () => {
        const rows = db.prepare('SELECT id, name, url, username, base_path FROM webdav_configs').all();
        return { configs: rows };
    });
    // 添加 WebDAV 配置
    app.post('/api/webdav', async (req, reply) => {
        const { name, url, username, password, base_path } = req.body;
        if (!name || !url) {
        ...
        const result = db.prepare('INSERT INTO webdav_configs (name, url, username, password, base_path, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(name, url, username || null, password || null, base_path || '/', Date.now());
        return {
    ...
    app.put('/api/webdav/:id', async (req, reply) => {
        ...
        const { name, url, username, password, base_path } = req.body;
        // 获取现有配置
        const existing = db.prepare('SELECT * FROM webdav_configs WHERE id = ?').get(Number(id));
        if (!existing) {
        ...
        db.prepare('UPDATE webdav_configs SET name = ?, url = ?, username = ?, password = ?, base_path = ?, updated_at = ? WHERE id = ?').run(newName, newUrl, newUsername, newPassword, newBasePath, Date.now(), Number(id));
        return { success: true };
    });
    // 删除 WebDAV 配置
    ...
    app.post('/api/webdav/:id/test', async (req, reply) => {
        const { id } = req.params;
        const config = db.prepare('SELECT * FROM webdav_configs WHERE id = ?').get(Number(id));
        if (!config) {
            return reply.code(404).send({ error: '配置不存在' });
        }
        try {
    ...
    app.get('/api/webdav/:id/browse', async (req, reply) => {
        ...
        try {
            ...
            const files = contents
                ...
                .filter((item) => /\.(mp3|flac|wav|ogg|m4a|aac|wma|ape)$/i.test(item.basename))
                .map((item) => ({
                name: item.basename,
                path: item.filename,
                isDirectory: false,
            ...
            return reply.code(500).send({ error: `浏览失败: ${err.message}` });
        }
    });
    // 获取 WebDAV 文件流（代理下载）
    app.get('/api/webdav/:id/stream', async (req, reply) => {
        ...
        try {
            ...
            return reply.code(500).send({ error: `获取文件失败: ${err.message}` });
        }
    });
}
