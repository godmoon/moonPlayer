...
export function Settings() {
  ...
  return (
    <div className="h-full overflow-auto p-3 md:p-4">
      <h2 className="text-lg md:text-xl font-bold mb-4 md:mb-6">设置</h2>
      ...
      <div className="mb-4 md:mb-6">
        ...
        <div className="space-y-2 mb-3">
          {musicPaths.length === 0 ? (
            ...
            musicPaths.map((path) => (
              <div key={path} className="flex items-center gap-2 p-2 bg-gray-700 rounded-lg">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <button
            ...
            className="px-4 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 rounded-lg text-sm shrink-0"
          >
            {saving ? '添加中...' : '添加'}
          </button>
        </div>
      ...
      <div className="mb-4 md:mb-6">
        ...
        <div className="space-y-2 mb-3">
          {webdavConfigs.length === 0 ? (
            ...
            webdavConfigs.map((config) => (
              <div key={config.id} className="flex items-center gap-2 p-2 bg-gray-700 rounded-lg">
                <span className="text-blue-500 shrink-0">☁️</span>
                <div className="flex-1 min-w-0">
                  <div className="text-sm truncate">{config.name}</div>
                  <div className="text-xs text-gray-500 truncate">{config.url}</div>
                </div>
                <div className="flex gap-1 shrink-0">
                  <button
                    onClick={() => handleTestWebdav(config.id)}
                    className="text-green-500 hover:text-green-400 text-xs px-1.5"
                  >
                    测试
                  </button>
                  <button
        ...
        {showWebdavForm ? (
          <div className="bg-gray-700 p-3 rounded-lg space-y-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                ...
                disabled={saving}
                className="flex-1 px-3 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 rounded-lg text-sm"
              >
                {saving ? '保存中...' : '保存'}
              </button>
              <button
          ...
          <button
            ...
            className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-sm"
          >
            ➕ 添加 WebDAV
          </button>
        )}
      ...
      {/* 省流模式 */}
      <div className="mb-4 md:mb-6">
        <label className="block text-sm text-gray-400 mb-2">省流模式(仅保存在本机)</label>
        <select
          value={qualityMode}
        ...
        <p className="text-xs text-gray-500 mt-1">
          选择非无损品质时，高比特率音频将转码为指定比特率，与设备相关，可以将车机设置为较低的品质以节省流量。无损品质将直接播放原始音频文件，不进行转码。
        </p>
      </div>
        ...
        {showPasswordForm ? (
          ...
          <div className="space-y-2">
            <button
              ...
              className="w-full px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm text-left"
            >
              🔐 修改密码
            </button>
            <button
              onClick={handleLogout}
              className="w-full px-4 py-2 bg-gray-700 hover:bg-red-600 rounded-lg text-sm text-left"
            >
              🚪 退出登录
            </button>
          </div>
      ...
      <div className="border-t border-gray-700 pt-3 md:pt-4">
        <h3 className="font-medium mb-2 text-sm md:text-base">关于</h3>
